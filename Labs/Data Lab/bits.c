...
int getByte(int x, int n) {
  int shiftAmount = n << 3;//this is to multiply the n value by 8 
  int byte = x >> shiftAmount; //to get the bytes that we want to be placed as the lowest byte
  int negativeOne = ~(0);
  return ~(negativeOne << 8) & byte; //mask it to only get that byte
}
/* 
...
int isLessOrEqual(int x, int y) {
  ...
  int difference = (~((y+(negativeX))>>31));
   return(!!(difference& (~differentSign))| !!(x_is_negative & differentSign));
}
/* 
...
int logicalShift(int x, int n) {
  int shiftAmount = 32 + (~n+1); //this is basically 32 - n but since - is not allowed, I used -n = ~n+1
  int negativeOne = ~0;
  int zero = (!!n)<< 31 >> 31;
  int mask = ~(negativeOne << shiftAmount & zero);
  return mask & x >> n;
}
/* 
